...
export const generateMealIdeas = async (
  ...
  logger.debug(
    ...
    geminiApiKey ? "Available" : "Not available"
  );
  logger.debug(
    "🔗 Supabase URL used for meal ideas generation:",
    supabase.supabaseUrl
  );
  logger.debug(
    "🔑 Checking supabaseKey:",
  ...
  logger.debug(
    "🍗 Freezer items being sent to generate meal ideas:",
  ...
  try {
    ...
    if (apiEndpoint) {
      ...
      if (!preferences) {
        logger.debug(
          "⚠️ No dietary preferences provided, fetching from user settings..."
        );
        try {
          ...
          if (user?.user) {
            ...
            if (settings?.dietary) {
              ...
              logger.debug(
                "👤 Using user dietary preferences from DB:",
                preferences
              );
            } else {
              logger.debug(
                "⚠️ No user dietary preferences found in DB, using defaults"
              );
              preferences = {
                vegetarian: false,
                vegan: false,
      ...
      const processedIdeas = data.map((idea: MealIdea) => {
        ...
        logger.debug(
          `Idea "${idea.title}" matched with: ${
            matchedItems.join(", ") || "none"
          }`
        );
      ...
      logger.debug(
        "⚠️ No valid API endpoint available, supabaseUrl is:",
        supabase.supabaseUrl
      );
    }
  } catch (error) {
  ...
  const mockIdeas: MealIdea[] = [
    {
      ...
      description:
        "Quick and easy stir fry with frozen vegetables and chicken.",
  ...
  logger.debug(
    "🔄 Returning mock ideas with matched items:",
    mockIdeas.map((idea) => ({
      title: idea.title,
...
const limitMealIdeas = async (userId: string): Promise<void> => {
  try {
    ...
    if (maxNonFavoritesToKeep <= 0) {
      logger.debug(
        "⚠️ All slots taken by favorites, no room for non-favorites"
      );
      return;
    }
    ...
    logger.debug(
      `🗑️ Deleting ${nonFavoritesToDelete.length} oldest non-favorite ideas`
    );
